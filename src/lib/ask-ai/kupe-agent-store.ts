...
function creationPrompt(orgId: string, projectId: string, userText: string): string {
  return [
    ...
    "Write a spoken-voice system_prompt (under ~1000 tokens, follow create_agent's voice-prompt rules: Identity→Closing in Title Case, You are {name}… persona, {{camelCase}} input variables in the prompt and greeting, config.variables declared) and a one-line greeting. Call set_agent_output_variables for any post-call targets. Do not call commit_agent_version unless the user explicitly asked to snapshot / save a version.",
...
function editPrompt(orgId: string, projectId: string, agentId: string, userText: string): string {
  return [
    "You are editing an existing voice agent -- use update_agent, never create_agent. Do not call commit_agent_version unless the user explicitly asked to commit / snapshot / save a version. If several uncommitted draft edits have piled up, recommend a snapshot in your reply and wait.",
